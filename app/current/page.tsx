...
export default function CurrentProgramsPage(){
  ...
  return <main className={styles.page}>
    <header className={styles.header}><Link href="/" className={styles.brand}><span/>ScenePin</Link><Link href="/search">全作品から探す</Link></header>
      <section className={styles.hero}><div><Link href="/" className={styles.back}>← ホームへ</Link><p>ON AIR · TOKYO</p><h1>放送中の番組<br/><span>ドラマ・バラエティ</span></h1><p className={styles.lead}>公式番組表と各局の公式ページで確認できた情報を掲載します。不明な項目は「確認中」のまま残します。</p></div><div className={styles.stats}><strong>{currentPrograms.length}</strong><span>PROGRAMS</span><b>2026年9月3日</b><small>東京地上波</small></div></section>
